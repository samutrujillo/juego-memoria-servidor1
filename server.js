...
let serviceAccount;
try {
    // Intentar cargar archivo de credenciales del servidor secundario
    serviceAccount = require('./firebase-credentials-secondary.json');
} catch (error) {
    // Si no existe, crear desde variables de entorno
    if (process.env.FIREBASE_CREDENTIALS_SECONDARY) {
        try {
            serviceAccount = JSON.parse(process.env.FIREBASE_CREDENTIALS_SECONDARY);
        } catch (parseError) {
            console.error('Error al parsear credenciales de Firebase secundario:', parseError);
        }
    } else {
        console.error('No se encontró el archivo de credenciales de Firebase secundario ni la variable de entorno');
    }
}
...
if (serviceAccount) {
    admin.initializeApp({
        ...
        databaseURL: "https://juego-memoria-secundario-default-rtdb.firebaseio.com/"
    });
    db = admin.database();
    console.log('Firebase secundario inicializado correctamente');
} else {
    console.warn('No se pudo inicializar Firebase secundario, se usará solo almacenamiento local');
}

const app = express();

// Rutas para archivos de estado con sistema de respaldo
const GAME_STATE_FILE = path.join(__dirname, 'game-state-secondary.json');
const GAME_STATE_BACKUP_1 = path.join(__dirname, 'game-state-secondary.backup1.json');
const GAME_STATE_BACKUP_2 = path.join(__dirname, 'game-state-secondary.backup2.json');
const ERROR_LOG_FILE = path.join(__dirname, 'error-log-secondary.txt');
...
const corsOptions = {
    origin: process.env.NODE_ENV === 'production'
        ? [
            process.env.CLIENT_URL || 'https://juego-memoria-cliente1.onrender.com',
            'https://juego-memoria-cliente1.onrender.com'
        ]
        : ['http://localhost:3000'],
    methods: ['GET', 'POST'],
    credentials: true
...
function queueGameStateChange(path, value) {
    ...
    if (!saveTimeout) {
        saveTimeout = setTimeout(() => {
            ...
            // Guardar cambios acumulados en Firebase secundario
            db.ref().update(updates)
                .then(() => console.log('Cambios incrementales guardados en Firebase secundario'))
                .catch(error => console.error('Error al guardar cambios incrementales en Firebase secundario:', error));
...
function checkScoreLimit(user) {
    ...
    if (user.score <= 23000 && !user.isAdmin) {
        ...
        if (!user.isLockedDueToScore) {
            ...
            if (db) {
                queueGameStateChange(`gameStateSecondary/userScores/${user.id}/isLockedDueToScore`, true);
            }
        ...
        if (db) {
            queueGameStateChange(`gameStateSecondary/userScores/${user.id}/isLockedDueToScore`, false);
        }
...
function verifyBlockingStates() {
    ...
    for (const user of users) {
        ...
        if (user.isLockedDueToScore !== shouldBeLockedDueToScore) {
            ...
            // Actualizar en Firebase
            if (db) {
                queueGameStateChange(`gameStateSecondary/userScores/${user.id}/isLockedDueToScore`, shouldBeLockedDueToScore);
            }
        }
    }
...
function adminResetTableCounters() {
    ...
    // Actualizar en Firebase si está disponible
    if (db) {
        queueGameStateChange('gameStateSecondary/lastTableResetDate', gameState.lastTableResetDate);
        queueGameStateChange('gameStateSecondary/tableCount', 0);

        const updates = {};
        Object.keys(playerTableCount).forEach(userId => {
            updates[`gameStateSecondary/userScores/${userId}/tablesPlayed`] = 0;
        });
...
function incrementTableCount(userId) {
    ...
    // Actualizar en Firebase si está disponible
    if (db) {
        queueGameStateChange(`gameStateSecondary/userScores/${userId}/tablesPlayed`, playerTableCount[userId]);
        queueGameStateChange('gameStateSecondary/tableCount', gameState.tableCount);
    }
...
async function saveGameState() {
    ...
    if (db) {
        try {
            await db.ref('gameStateSecondary').set(stateToSave);
...
async function loadGameState() {
    ...
    if (db) {
        try {
            ...
            const snapshot = await db.ref('gameStateSecondary').once('value');
    ...
    if (loadedState) {
        ...
        if (loadedSource !== 'Firebase' && db) {
            ...
            try {
                await db.ref('gameStateSecondary').set(loadedState);
    ...
    if (db) {
        try {
            ...
            await db.ref('gameStateSecondary').set(initialState);
...
function generateBoard() {
   ...
   for (let row = 0; row < 4; row++) {
       ...
       for (let i = 0; i < 2; i++) {
           rowTiles.push({ value: 20000, revealed: false });  // Ganadora
       }
       for (let i = 0; i < 2; i++) {
           rowTiles.push({ value: -20000, revealed: false }); // Perdedora
       }
...
function updateUserScore(id, points) {
   ...
   if (user) {
       ...
       // Actualizar en Firebase si está disponible
       if (db) {
           queueGameStateChange(`gameStateSecondary/userScores/${id}/score`, user.score);
           queueGameStateChange(`gameStateSecondary/userScores/${id}/prevScore`, user.prevScore);
       }
...
function initPlayerSelections(userId) {
   if (!gameState.playerSelections[userId]) {
       ...
       // Actualizar en Firebase si está disponible
       if (db) {
           queueGameStateChange(`gameStateSecondary/playerSelections/${userId}`, gameState.playerSelections[userId]);
       }
   }
...
function verifyAndFixGameState() {
   ...
   for (let i = 0; i < gameState.board.length; i++) {
       ...
       if (!gameState.board[i] || gameState.board[i].value === undefined) {
           gameState.board[i] = { 
               value: (Math.random() > 0.5 ? 20000 : -20000), 
               revealed: false 
           };
           fichasCorregidas++;
       }
...
async function resetBoardOnly() {
   ...
   if (db) {
       try {
           const criticalUpdates = {
               'gameStateSecondary/board': gameState.board,
               'gameStateSecondary/globalTableNumber': globalTableNumber,
               'gameStateSecondary/rowSelections': gameState.rowSelections,
               'gameStateSecondary/playerSelections': gameState.playerSelections,
               'gameStateSecondary/players': gameState.players.map(p => ({
                   id: p.id,
                   username: p.username,
                   socketId: p.socketId,
               ...
               'gameStateSecondary/currentPlayerIndex': gameState.currentPlayerIndex
           };
   ...
   for (const player of gameState.players) {
       ...
       if (db) {
           queueGameStateChange(`gameStateSecondary/userScores/${playerId}/tablesPlayed`, playerTableCount[playerId]);
       }
...
async function resetGame() {
   ...
   if (db) {
     try {
       ...
       const resetUpdates = {
         'gameStateSecondary/board': gameState.board,
         'gameStateSecondary/status': 'resetCompleted', // Marcar específicamente como resetCompleted
         'gameStateSecondary/globalTableNumber': 1,
         'gameStateSecondary/rowSelections': [0, 0, 0, 0],
         'gameStateSecondary/playerSelections': gameState.playerSelections, // Incluir todas las selecciones reiniciadas
         'gameStateSecondary/turnStartTime': Date.now()
       };
       
       // Añadir reset de puntajes de todos los usuarios
       users.forEach(user => {
         if (!user.isAdmin) {
           resetUpdates[`gameStateSecondary/userScores/${user.id}/score`] = 60000;
           resetUpdates[`gameStateSecondary/userScores/${user.id}/prevScore`] = 60000;
           resetUpdates[`gameStateSecondary/userScores/${user.id}/isBlocked`] = false;
           resetUpdates[`gameStateSecondary/userScores/${user.id}/isLockedDueToScore`] = false;
           resetUpdates[`gameStateSecondary/userScores/${user.id}/tablesPlayed`] = 0;
         }
       });
...
function startPlayerTurn() {
   ...
   // Actualizar en Firebase si está disponible
   if (db) {
       queueGameStateChange('gameStateSecondary/currentPlayerIndex', gameState.currentPlayerIndex);
       queueGameStateChange('gameStateSecondary/status', 'playing');
       queueGameStateChange('gameStateSecondary/turnStartTime', gameState.turnStartTime);
       queueGameStateChange('gameStateSecondary/rowSelections', gameState.rowSelections);
   }
...
io.on('connection', (socket) => {
   ...
   socket.on('reconnectUser', ({ userId, username }) => {
       ...
       if (playerIndex !== -1) {
           ...
           // Actualizar en Firebase si está disponible
           if (db) {
               queueGameStateChange(`gameStateSecondary/players/${playerIndex}/isConnected`, true);
               queueGameStateChange(`gameStateSecondary/players/${playerIndex}/socketId`, socket.id);
           }
   ...
   socket.on('resetRowSelections', ({ userId }) => {
       ...
       // Actualizar en Firebase
       if (db) {
           queueGameStateChange(`gameStateSecondary/playerSelections/${userId}/rowSelections`, [0, 0, 0, 0]);
           queueGameStateChange(`gameStateSecondary/playerSelections/${userId}/totalSelected`, 0);
       }
   });
   ...
   socket.on('saveGameState', async ({ userId }) => {
       ...
       if (db) {
           try {
               await db.ref(`gameStateSecondary/playerGameStates/${userId}`).set(playerGameState[userId]);
   ...
   socket.on('joinGame', () => {
       ...
       if (existingPlayerIndex === -1) {
           ...
           if (db) {
               ...
               queueGameStateChange(`gameStateSecondary/players/${newPlayerIndex}`, {
                   id: userId,
                   username: user.username,
                   socketId: socket.id,
                   isConnected: true
           ...
           // Actualizar en Firebase si está disponible
           if (db) {
               queueGameStateChange(`gameStateSecondary/players/${existingPlayerIndex}/isConnected`, true);
               queueGameStateChange(`gameStateSecondary/players/${existingPlayerIndex}/socketId`, socket.id);
           }
...
socket.on('unlockAllTables', async (_, callback) => {
   ...
   socket.on('selectTile', async ({ tileIndex, currentScore }) => {
       ...
       if (db) {
           try {
               const criticalUpdates = {
                   [`gameStateSecondary/board/${tileIndex}/revealed`]: true,
                   [`gameStateSecondary/board/${tileIndex}/selectedBy`]: user.username,
                   [`gameStateSecondary/board/${tileIndex}/selectedAt`]: Date.now(),
                   [`gameStateSecondary/playerSelections/${userId}/rowSelections/${row}`]: playerSelections.rowSelections[row],
                   [`gameStateSecondary/playerSelections/${userId}/totalSelected`]: playerSelections.totalSelected,
                   [`gameStateSecondary/rowSelections`]: gameState.rowSelections
               };
       ...
       if (db) {
           try {
               await db.ref(`gameStateSecondary/userScores/${userId}`).update({
                   score: newScore,
                   prevScore: oldScore
               });
   ...
   socket.on('toggleBlockUser', async ({ userId }, callback) => {
       ...
       if (db) {
           try {
               await db.ref(`gameStateSecondary/userScores/${userId}/isBlocked`).set(targetUser.isBlocked);
   ...
   socket.on('unlockUserScore', async ({ userId }, callback) => {
       ...
       if (db) {
           try {
               await db.ref(`gameStateSecondary/userScores/${userId}/isLockedDueToScore`).set(false);
...
socket.on('changeUsername', async ({ userId, newUsername }, callback) => {
   ...
   if (db) {
       try {
           await db.ref(`gameStateSecondary/userScores/${userId}/username`).set(newUsername);
           if (playerIndex !== -1) {
               await db.ref(`gameStateSecondary/players/${playerIndex}/username`).set(newUsername);
           }
...
socket.on('changePassword', async ({ userId, newPassword }, callback) => {
   ...
   if (db) {
       try {
           await db.ref(`gameStateSecondary/userScores/${userId}/passwordChanged`).set(true);
           await db.ref(`gameStateSecondary/userScores/${userId}/passwordChangedAt`).set(Date.now());
   ...
   socket.on('unlockTables', async ({ userId }, callback) => {
       ...
       if (db) {
           try {
               await db.ref(`gameStateSecondary/userScores/${userId}/tablesPlayed`).set(0);
   ...
   socket.on('leaveGame', () => {
       ...
       if (playerIndex !== -1) {
           ...
           // Actualizar en Firebase
           if (db) {
               queueGameStateChange(`gameStateSecondary/players/${playerIndex}/isConnected`, false);
           }
           ...
           // Actualizar en Firebase
           if (db) {
               queueGameStateChange(`gameStateSecondary/playerGameStates/${userId}`, playerGameState[userId]);
           }
