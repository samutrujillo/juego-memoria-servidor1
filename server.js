...
if (serviceAccount) {
   admin.initializeApp({
       ...
       databaseURL: process.env.FIREBASE_DATABASE_URL || "https://juego-memoria-secundario-default-rtdb.firebaseio.com/"
   });
   db = admin.database();
...
const corsOptions = {
   origin: process.env.NODE_ENV === 'production'
       ? [
           ...
           'https://juego-memoria-cliente1.onrender.com' // Sin la barra al final
       ]
       : ['http://localhost:3000'],
   methods: ['GET', 'POST'],
   credentials: true
...
function loadUsers() {
   try {
       if (fs.existsSync(USERS_FILE)) {
           ...
           console.log('Usuarios cargados desde archivo:', loadedUsers.length);
           return loadedUsers;
       }
   } catch (error) {
   ...
   // Si no se pueden cargar, devolver usuarios por defecto
   return [
...
io.on('connection', (socket) => {
   ...
   socket.on('changeUsername', async ({ userId, newUsername }, callback) => {
       ...
       // NUEVO: Guardar usuarios inmediatamente para persistencia
       await saveUsers();
...
console.log('URL del cliente:', process.env.CLIENT_URL || 'https://juego-memoria-cliente1.onrender.com');
console.log('Firebase:', db ? 'Configurado' : 'No configurado');
console.log('Base de datos Firebase:', 'https://juego-memoria-secundario-default-rtdb.firebaseio.com/');
